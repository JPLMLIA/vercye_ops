...
def create_region_dir(base_dir, region_name, yield_value=10000, apsim_mean_yield=3000, lai_value=3.5):
    ...
    # Yield estimate CSV
    pd.DataFrame({"total_yield_production_kg": [yield_value], "total_area_ha": [100.0]}).to_csv(
        os.path.join(region_dir, f"{region_name}_converted_map_yield_estimate.csv"), index=False
    )
    ...
    # LAI stats CSV
    pd.DataFrame({"interpolated": [0, 0, 1], "Cloud or Snow Percentage": [20.0, 50.0, 100.0]}).to_csv(
        os.path.join(region_dir, f"{region_name}_LAI_STATS.csv"), index=False
    )
