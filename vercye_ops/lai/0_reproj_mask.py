...
def find_union_extent_LAI_info(
    lai_dir: str,
    lai_region: str,
    lai_resolution: int,
    lai_file_ext: rio.coords.BoundingBox,
    start_date=None,
    end_date=None,
):
    ...
    if not lai_files:
        raise Exception(
            f"No LAI files found in {lai_dir} for region {lai_region} with resolution {lai_resolution} and extension {lai_file_ext}"
        )
        ...
        with rio.open(lai_file) as ds:
            if union_bounds is None:
                ...
                if ds.res != (res_x, res_y):
                    raise Exception(
                        f"LAI files have different resolutions: {ds.res} vs {(res_x, res_y)}."
                    )
...
def handle_identify_extent_reproject(
    lai_dir,
    lai_region,
    lai_resolution,
    lai_file_ext,
    ...
    union_bounds, lai_crs, (res_x, res_y) = find_union_extent_LAI_info(
        lai_dir, lai_region, lai_resolution, lai_file_ext, start_date, end_date
    )
...
@click.option(
    ...
    default=None,
    help="Directory where LAI data is saved. Needs to be used with --lai_region.",
)
@click.option(
    "--lai_region",
    type=str,
    default=None,
    help="Region of LAI data to use. Needs to be used with --lai_dir.",
)
@click.option(
    "--lai_resolution",
    type=int,
    help="Resolution of LAI data to use. Needs to be used if providing --lai_dir",
)
@click.option(
    "--lai_file_ext",
    type=click.Choice(["tif", "vrt"]),
    help="File extension of the LAI files. Usage with --lai_dir",
    default="tif",
)
@click.option(
    "--lai_path",
    type=click.Path(exists=True),
    default=None,
    help="Path to a specific LAI file. Mutually exclusive with --lai_dir/region.",
)
@click.option(
    "--start_date",
    type=click.DateTime(formats=["%Y-%m-%d"]),
    default=None,
    help="Start date for LAI data.",
)
@click.option(
    "--end_date",
    type=click.DateTime(formats=["%Y-%m-%d"]),
    default=None,
    help="End date for LAI data.",
)
def main(
    mask_path,
    out_path,
    ...
    if lai_dir and lai_region:
        ...
        handle_identify_extent_reproject(
            lai_dir,
            lai_region,
            lai_resolution,
            lai_file_ext,
