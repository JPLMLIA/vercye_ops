...
def csv_to_weight_dict(csvpath, product):
    model_ids = {"LAI": 1, "fAPAR": 2, "fCOVER": 3, "CCC": 4, "CWC": 5, "Albedo": 6, "DASF": 7}
...
def weight_dict_to_torch(w_dict, n_ch_in, outpath):
    ...
    with torch.no_grad():
        ...
        model.output.weight = nn.Parameter(torch.tensor(w_dict["output"]["weights"]).reshape(1, 1, 1, 1))
        model.output.bias = nn.Parameter(torch.tensor(w_dict["output"]["biases"]).reshape(1, 1, 1, 1))
...
@click.argument("model", type=click.Choice(["LAI", "fAPAR", "fCOVER", "CCC", "CWC", "Albedo", "DASF"]))
