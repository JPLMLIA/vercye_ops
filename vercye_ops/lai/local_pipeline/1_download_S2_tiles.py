...
from vercye_ops.lai.local_pipeline.stac_downloader import run_pipeline
...
def add_geometry_bands(item, band_paths, band_names, resolution, mask, metadata_file_paths, output_folder):
...
@click.command()
@click.option("--satellite", type=click.Choice(['S2'], case_sensitive=False), default='S2', help="Satellite source to use. Currently supported 'S2' (Sentinel-2)")
...
@click.option(
    "--resolution", type=int, help="Target resolution in meters."
)
@click.option(
    "--geojson-path",
...
@click.option(
    ...
    type=int,
    default=50,
    help="Threshold for pixels to be considered cloud.",
)
@click.option(
    "--snowprob-thresh",
    type=int,
...
def main(
    ...
    if satellite == 's2':
        ...
        mask_processor = build_s2_mask_processor(cloud_thresh=cloudprob_thresh, snowprob_thresh=snowprob_thresh)
        band_processor = add_geometry_bands

        # 'cosVZA', 'cosSZA', 'cosRAA' will be prepended by the cutom band processor
        if resolution == 10:
            # ['B2', 'B3', 'B4', 'B8']
