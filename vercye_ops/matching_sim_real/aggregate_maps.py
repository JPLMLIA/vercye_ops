        ...
        with rio.open(file_paths[0]) as first_ds:
            ...
            if reference_crs is None:
                ...
                if product_crs != reference_crs:
                    raise ValueError(
                        f"CRS mismatch detected in {label} TIF files: {product_crs} vs {reference_crs}"
                    )
...
def merge_tifs(tif_files, label):
    ...
    band_names = [
        (
            f"{label}_{datasets[0].descriptions[i]}"
            if datasets[0].descriptions[i]
            else f"{label}_Band_{i+1}"
        )
...
def cli(
    ...
    merged_gdf = merge_shapefiles(
        [get_file_path(roi_base_dir, region, ".geojson") for region in regions], regions
    )
    ...
    if val_fpath:
        ...
        merged_gdf = merged_gdf.merge(
            val_data, left_on="cleaned_region_name_vercye", right_on="region"
        )
