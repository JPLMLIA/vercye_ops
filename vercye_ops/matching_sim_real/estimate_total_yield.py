...
def estimate_yield(tif_path, output_yield_csv_fpath, target_crs):
    ...
    with rasterio.open(tif_path) as src:
        ...
        pixel_area_m2 = compute_pixel_area(
            center_lon, center_lat, pixel_width_deg, pixel_height_deg, target_crs
        )
        ...
        logger.info(
            f"Pixel area: {pixel_area_m2:0.4f} square meters, {pixel_area_ha:0.6f} hectares"
        )
        ...
        pd.DataFrame(
            {
                ...
                "total_yield_production_ton": [total_yield_tons],
            }
...
@click.command()
@click.option(
    "--converted_lai_tif_fpath",
    required=True,
    type=click.Path(exists=True),
    help="Filepath to the input converted LAI geotiff file.",
)
@click.option(
    "--target_crs",
    required=True,
    help='CRS string (proj4/authority e.g "EPSG:1234") of the project target coordinate system (used for pixel area calculation).',
)
@click.option(
    "--output_yield_csv_fpath",
    required=True,
    type=click.Path(),
    help="Filepath where the total yield will be saved as a CSV file.",
)
