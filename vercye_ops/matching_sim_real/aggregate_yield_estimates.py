    ...
    for region_dir in Path(yield_dir).iterdir():
        if region_dir.is_dir():
            ...
            if yield_estimate_csv_path.exists():
                ...
                logger.warning(
                    f"Converted map yield estimate CSV file not found for region: {region_name}. Dropping from Output."
                )
                continue
            ...
            if conv_factor_csv_path.exists():
                ...
                conv_df["apsim_all_std_yield_estimate_kg_ha"] = conv_df["apsim_all_std_yield_estimate_kg_ha"].astype(
                    int
                )
            ...
            if columns_to_keep is not None and not combined_df.empty:
                ...
                if geojson_path.exists():
                    ...
                    if missing_columns:
                        raise Exception(
                            f"Columns {missing_columns} not found in the geojson file for region: {region_name}. Please ensure the columns are named correctly."
                        )
...
@click.option(
    ...
    help="Path to the yield directory (e.g., year, timepoint) containing region subdirectories.",
)
@click.option(
    "--output_csv",
    required=True,
    type=click.Path(),
    help="Path to save the aggregated yield estimates CSV.",
)
@click.option(
    "--columns_to_keep",
    required=False,
    default=None,
    type=str,
    help="Comma-separated list of geojsons-columns to keep in the output CSV. If not provided, no additional columns from the geojsons will be kept.",
)
@click.option(
    "--chirps_file",
    required=False,
    type=click.Path(exists=True),
    default=None,
    help="Path to the CHIRPS Parquet file for precipitation data. Used to identify if fallback or chirps was used for precipitation.",
)
@click.option(
    "--referencedata_fpath",
    required=False,
    type=click.Path(exists=True),
    help="Filepath to reference data csv at this aggregation level, containing region and reported_mean_yield_kg_ha column.",
)
@click.option("--verbose", is_flag=True, help="Enable verbose logging.")
def cli(yield_dir, output_csv, columns_to_keep, chirps_file, referencedata_fpath, verbose):
    """Aggregate yield estimates from multiple regions within a directory."""
    if verbose:
