    ...
    for region_dir in Path(yield_dir).iterdir():
        if region_dir.is_dir():
            ...
            if yield_estimate_csv_path.exists():
                ...
                yield_df["total_yield_production_kg"] = yield_df[
                ...
                logger.warning(
                    f"Converted map yield estimate CSV file not found for region: {region_name}"
                )
            ...
            if conv_factor_csv_path.exists():
                ...
                conv_df["apsim_all_std_yield_estimate_kg_ha"] = conv_df[
                    "apsim_all_std_yield_estimate_kg_ha"
                ].astype(int)
            ...
            if columns_to_keep is not None and not combined_df.empty:
                ...
                if geojson_path.exists():
                    ...
                    missing_columns = [
                        col for col in columns_to_keep_list if col not in gdf.columns
                    ]
                    if missing_columns:
                        raise Exception(
                    ...
                    raise Exception(
                        f"Geojson file not found for region: {region_name}. Please ensure the file exists."
                    )
