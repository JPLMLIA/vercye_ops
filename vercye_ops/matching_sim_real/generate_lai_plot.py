...
def create_lai_traces(df, region_name, lai_type_col, adjusted_color='blue', non_adjusted_color='orange'):
...
def generate_lai_figure(filepaths, lai_agg_type):
    ...
    for filepath in filepaths:
        ...
        adj_trace, non_adj_trace = create_lai_traces(df, region_name, lai_type_col,
                                                     adjusted_color, non_adjusted_color)
    ...
    fig.update_layout(
        updatemenus=[
            dict(
                ...
                direction="right",
                buttons=[
                    dict(args=[{"visible": all_visible}], label="Show All", method="update"),
                    dict(args=[{"visible": hide_adjusted}], label="Hide Adjusted", method="update"),
                    dict(args=[{"visible": hide_non_adjusted}], label="Hide Non-Adjusted", method="update")
                ],
                pad={"r": 10, "t": 10},
                showactive=True,
                x=1,
