...
def create_multi_band_tif(single_year_data, multi_year_data, year, num_last_years, reference_tif, out_tif_path):
    ...
    transform, crs, (height, width) = ref_meta['transform'], ref_meta['crs'], ref_meta['shape']

    variables = ['rain', 'meant', 'maxt', 'mint', 'radn', 'wind']
    descriptions = ['Annual Rainfall', 'Mean Temperature', 'Max Temperature', 'Min Temperature', 'Radiation', 'Wind Speed']
...
@click.option('--reference_tif_path', required=True, type=click.Path(), default=None, help='Path to a tif file containing the target crs and resolution.')
@click.option('--output_tif_path', required=True, type=click.Path(), default=None, help='Path to the output .tif file.')
@click.option('--verbose', is_flag=True, help='Enable verbose logging.')
def cli(regions_base_dir, output_pdf_path, year, num_last_years, reference_tif_path, output_tif_path, verbose):
    """Aggregate meteorological stats from multiple regions within a directory."""
    if verbose:
    ...
    if output_tif_path and reference_tif_path:
        ...
        create_multi_band_tif(single_year_stats, multiyear_stats, year, num_last_years, reference_tif_path, output_tif_path)                                                                                           
