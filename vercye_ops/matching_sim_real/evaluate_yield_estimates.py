...
def get_preds_obs(estimation_fpath, val_fpath):
    ...
    if "mean_yield_kg_ha" not in combined.columns:
        raise ValueError(
            "Column 'mean_yield_kg_ha' not found in the input estimation csv. Please ensure that the columns are named correctly."
        )
    ...
    if "reported_mean_yield_kg_ha" not in combined.columns:
        if "reported_production_kg" not in combined.columns:
            raise ValueError(
                "Could not compute metrics as neither 'reported_mean_yield_kg_ha' or 'reported_production_kg' are not available in the input csv."
            )

        combined["reported_mean_yield_kg_ha"] = (
            combined["total_area_ha"] / combined["reported_production_kg"]
        )
...
@click.option(
    ...
    help="Filepath to the csv containing the refernece data per region.",
)
@click.option(
    "--estimation_fpath",
    required=True,
...
@click.option(
    ...
    required=True,
    type=click.Path(),
    help="Filepath where the resulting metrics should be saved (.csv).",
)
@click.option(
    "--out_plot_fpath",
    required=True,
