...
def load_prep_project_data(
    weather_data_fpath, sim_end_date, precipitation_src, fallback_precipitation, region, chirps_file
):
    """
...
def create_met_file(
    ...
    with open(output_fpath, "w") as file:
        ...
        header_units = " ".join(
            [f"{APSIM_UNITS[POWER_TO_APSIM[key]]:<8}" for key in df.columns if key in POWER_TO_APSIM]
        )
        file.write(f"{header}\n")
