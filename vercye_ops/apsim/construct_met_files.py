...
def load_chirps_precipitation(start_date, end_date, chirps_file, column_name):
    ...
    if not missing_dates.empty:
        raise ValueError(
            f"CHIRPS data is missing for the following dates: {missing_dates.tolist()}"
        )

    return chirps_data[column_name]


def load_prep_project_data(
    weather_data_fpath,
    sim_end_date,
    precipitation_src,
    ...
    if precipitation_src.lower() == "chirps":
        ...
        try:
            ...
            if not fallback_precipitation:
                raise KeyError(
                    "CHIRPS region data not found and fallback precipitation is not enabled."
                )
        except ValueError:
            if not fallback_precipitation:
    ...
    if sim_end_date.year != last_date.year:
        logger.warning(
            "Attempting to project weather data beyond the current year. You likely want data and simulation end date to be in the same year."
        )
    ...
    proj_dicts = [
        yearly_avg_data.loc[pd].to_dict() for pd in proj_days
    ]  # index into projection data, now indexed by DOY
...
def create_met_file(
    ...
    with open(output_fpath, "w") as file:
        ...
        file.write(
            f"amp = {amp:5.2f} (oC)    ! annual amplitude (i.e., range) in mean monthly temperature\n"
        )

        # Write header
        header = " ".join(
            [f"{POWER_TO_APSIM[key]:<8}" for key in df.columns if key in POWER_TO_APSIM]
        )
        header_units = " ".join(
            [
                f"{APSIM_UNITS[POWER_TO_APSIM[key]]:<8}"
                for key in df.columns
                if key in POWER_TO_APSIM
            ]
        )
        file.write(f"{header}\n")
...
@click.option(
    ...
    required=True,
    help="Path to the CSV file containing weather data.",
)
...
@click.option(
    ...
    required=True,
    help="End date of the simulation period.",
)
@click.option(
    "--chirps_file",
    type=click.Path(exists=True),
    required=False,
    help="Path to the precomputed CHIRPS data file.",
)
@click.option(
    "--precipitation_source",
    type=click.Choice(["chirps", "ERA5", "nasa_power"], case_sensitive=False),
    default="nasa_power",
    show_default=True,
    help="Source of precipitation data.",
)
@click.option(
    "--fallback_precipitation",
    help="Fallback to original precipitation data if CHIRPS data is not available.",
    default=False,
)
@click.option(
    "--output_dir",
    ...
    required=True,
    help="File path for the .met output file.",
)
