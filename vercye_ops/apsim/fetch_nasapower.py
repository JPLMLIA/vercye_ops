...
def fetch_nasa_power_data(start_date, end_date, variables, lon, lat):
    ...
    logger.info(
        "Fetching data from NASA POWER for %s to %s...",
        start_date.date(),
        end_date.date(),
    )
...
def fetch_from_cache(cache_fpath, start_date, end_date, variables, lon, lat):
    ...
    if len(set(nodata_vals)) > 1:
        raise ValueError(
            "Nodata values are inconsistent across fetched data chunks - Not handling this case."
        )
...
@click.option(
    "--variables",
    type=click.Choice(VALID_CLIMATE_VARIABLES, case_sensitive=False),
    multiple=True,
    default=DEFAULT_CLIMATE_VARIABLES,
    show_default=True,
    help="Meteorological variables to fetch for NasaPower. Unused if met source is ERA5",
)
...
def cli(
    ...
    if cache_dir is not None and Path(cache_fpath).exists() and not overwrite_cache:
        logger.info(
            "Weather data already exists locally. Will fetch and append only missing dates if necessary to : \n%s",
            cache_fpath,
        )
        df = fetch_from_cache(cache_fpath, start_date, end_date, variables, lon, lat)
    else:
        logger.info(
            "Fetching weather data from NASA POWER for %s to %s",
            start_date.date(),
            end_date.date(),
        )
