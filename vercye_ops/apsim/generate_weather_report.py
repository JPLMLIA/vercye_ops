...
def create_plots(df, metadata):
    ...
    def add_traces(entire_df, data_type, fig):
        ...
        fig.add_trace(go.Scatter(x=df['date'], y=df['maxt'], mode='lines', name=f'Max Temp ({data_type})',
                                 line=dict(color='red', dash=line_dash)), row=1, col=1)
        fig.add_trace(go.Scatter(x=df['date'], y=df['mint'], mode='lines', name=f'Min Temp ({data_type})',
                                 line=dict(color='blue', dash=line_dash)), row=1, col=1)
        fig.add_trace(go.Scatter(x=df['date'], y=df['meant'], mode='lines', name=f'Mean Temp ({data_type})',
                                 line=dict(color='black', dash=line_dash)), row=1, col=1)

        # Radiance
        fig.add_trace(go.Scatter(x=df['date'], y=df['radn'], mode='lines', name=f'Radiance ({data_type})',
                                 line=dict(color='goldenrod', dash=line_dash)), row=2, col=1)

        # Precipitation
        fig.add_trace(go.Scatter(x=df['date'], y=df['rain'], mode='lines', name=f'Rain ({data_type})',
                                 line=dict(color='dodgerblue', dash=line_dash)), row=3, col=1)

        # Wind
        fig.add_trace(go.Scatter(x=df['date'], y=df['wind'], mode='lines', name=f'Surface Wind ({data_type})',
                                 line=dict(color='purple', dash=line_dash)), row=4, col=1)
    ...
    # Update layout for hover and linking subplots
    fig.update_layout(
        hovermode='x unified',
        title_text=f"<b>Weather Data</b><br>Input file: <i>{metadata['input_fpath']}</i><br>Lat {metadata['latitude']}, Lon {metadata['longitude']}, Tav {metadata['tav']}°C, Amp {metadata['amp']}°C<br>",
