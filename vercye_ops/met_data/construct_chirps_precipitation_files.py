...
def construct_chirps_precipitation_file(dates, aggregation_method, regions_base_dir, chirps_dir, version):
    ...
    region_geometry_files = [
        op.join(regions_base_dir, region_name, f"{region_name}.geojson") for region_name in region_names
    ]
    ...
    if aggregation_method == "centroid":
        ...
        valid_indices = [
            i for i, c in enumerate(centroids_unfiltered) if is_coord_within_bounds(c[0], c[1], chirps_bounds)
        ]
        ...
        valid_indices = [
            i for i, gdf in enumerate(region_gdfs_unfiltered) if is_bbox_within_bounds(gdf.total_bounds, chirps_bounds)
        ]
