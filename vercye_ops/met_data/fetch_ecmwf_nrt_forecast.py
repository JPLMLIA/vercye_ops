...
from vercye_ops.met_data.fetch_era5 import build_ee_geometry, clean_era5, error_checking_function, postprocess_for_apsim, write_met_data_to_csv
...
def fetch_nrt_forecast(forecasting_date, lon, lat, polygon_path):
    ...
    def daily_chunk(offset):
        ...
        img = aggregate_daily(
            era5.filter(ee.Filter.rangeContains("forecast_hours", offset, offset.add(23)))
        )
...
@click.command()
@click.option('--forecast_date', type=click.DateTime(formats=["%Y-%m-%d"]), help="End date for data collection in YYYY-MM-DD format.")
@click.option('--forecast_date_from_existing', is_flag=True, help="Instead of using the fixed forecast_date, derive the forecast date as the last date in an existing output_file.")
...
@click.option('--output_file', type=click.Path(file_okay=True, dir_okay=False, exists=True), required=True, help="File where the forecasts should be saved. Will append by default if existing.")
@click.option('--overwrite', is_flag=True, help="If the output_file alraedy exists, data will not be appended but overwritten.")
@click.option('--verbose', is_flag=True, help="Enable verbose logging.")
def cli(forecast_date, forecast_date_from_existing, lon, lat, polygon_path, met_agg_method, ee_project, output_file, overwrite, verbose):
    ...
    if forecast_date_from_existing and not Path(output_file).exists():
        raise Exception(f"Forecast_date_from_existing chosen, but no file to derive the last date exists under {output_file}.")
    ...
    if met_agg_method == 'mean':
        ...
        df = fetch_nrt_forecast(
            forecasting_date=forecast_date, 
            lon=None,
            lat=None,
