...
class InteractiveMapGenerator:
    ...
    def create_geojson_level(self, level_idx: int) -> Dict[str, Any]:
        ...
        for idx, row in geometries.iterrows():
            ...
            if region_name in self.aggregated_estimates[level_idx]:
                ...
                median_estimated_yield_kg_ha = self.aggregated_estimates[level_idx][region_name][
    ...
    def create_hierarchical_data(self) -> Dict[str, Any]:
        ...
        """
        levels = {"levels": [], "level_data": {}, "level_mappings": {}}

        # Add level information
        for i, level in enumerate(self.aggregation_levels):
            levels["levels"].append({"index": i, "name": level["name"], "column": level["column"]})
        ...
        if len(self.aggregation_levels) > 1:
            for i in range(len(self.aggregation_levels) - 1):
                ...
                for feature in parent_level["features"]:
                    ...
                    child_features = [
                        f for f in next_level_data["features"] if f["properties"]["name"] in parent_regions
                    ]
    ...
    def compute_value_ranges(self, levels):
        ...
        for valueType in [
            ...
            value_ranges[valueType] = {"min_val": min_val, "max_val": max_val, "p95_val": p95_val}
            ...
            <body>
                ...
                <div id="detailOverlay" class="detail-overlay">
                    <div class="detail-overlay-content">
                        ...
                        <div class="detail-overlay-stats">
                            <div class="detail-overlay-stat-item">
                                <div class="detail-overlay-stat-value" id="overlayValue4">0.0</div>
                                <div class="detail-overlay-stat-label">Reported Mean Yield (kg/ha)</div>
                            </div>
                            <div class="detail-overlay-stat-item">
                                <div class="detail-overlay-stat-value" id="overlayValue5">0.0</div>
                                <div class="detail-overlay-stat-label">Absolute Error (kg/ha)</div>
                            </div>
                            <div class="detail-overlay-stat-item">
                                <div class="detail-overlay-stat-value" id="overlayValue6">0.0</div>
                                <div class="detail-overlay-stat-label">Relative Error (%)</div>
                            </div>
                        </div>
                        <hr></hr>
    ...
    def load_aggregated_estimates(self):
        ...
        for idx, level in enumerate(self.aggregation_levels):
            ...
            agg_estimates_df["region"] = agg_estimates_df["region"].astype(str)
    ...
    def load_sim_data(self):
        ...
        for sim_png in sim_pngs:
            ...
            if region_name in all_sim_data:
                raise ValueError(
                    f"Duplicate report for '{region_name}':\n" f"  {all_sim_data[region_name]}\n  {sim_png}"
                )
...
@click.option(
    ...
    help="Path to the shapefile (.geojson).",
)
@click.option(
    "--basedir-path",
    required=True,
    type=click.Path(exists=True),
    help="Path to the base directory of the timepoint in the study.",
)
...
@click.option(
    "--lai-column",
    multiple=True,
    default=DEFAULT_LAI_COLUMNS,
    callback=parse_lai_column,
...
def main(
    ...
    if smoothed:
        ...
        lai_column_names = [name + " Smoothed" for name in lai_column_names] + [
            name + " Unsmoothed" for name in lai_column_names
        ]
