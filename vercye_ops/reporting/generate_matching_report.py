...
def generate_report(
    ...
    for _, row in apsim_filtered.iterrows():
        ...
        fig.add_trace(
            go.Scatter(
                x=sim_data.index,
                y=sim_data[f"{crop_name}.Leaf.LAI"],
                mode="lines",
                name=f"LAI/Yield (Filtered on Step: {legend_group})",
                opacity=style["opacity"],
                zorder=style["zorder"],
                legendgroup=legend_group,
                showlegend=style["show_group"],
                line=dict(color=style["line_color"], width=style["line_width"]),
                hovertemplate=("Date: %{x|%Y-%m-%d}<br>" "LAI: %{y:.2f}<br>" f"SimulationID: {sim_id}<extra></extra>"),
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=sim_data["Yield"],
                mode="lines",
                name=f"Yield (Filtered on Step: {legend_group})",
                opacity=style["opacity"],
                zorder=style["zorder"],
                legendgroup=legend_group,
                showlegend=False,
                line=dict(color=style["line_color"], width=style["line_width"]),
                hovertemplate=("Date: %{x|%Y-%m-%d}<br>" "LAI: %{y:.2f}<br>" "SimulationID: %{sim_id}<extra></extra>"),
            ),
            row=2,
            col=1,
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=rs_df[lai_column],
            mode="lines",
            name=f"RS {lai_agg_type_name} LAI",
    ...
    fig.add_trace(
        go.Scatter(
            x=non_interp.index,
            y=non_interp[observed_dots_col],
            mode="markers",
            name="RS Observed (non-interpolated)",
    ...
    if lai_column + " Unsmoothed" in rs_df.columns:
        fig.add_trace(
            go.Scatter(
                x=rs_df.index,
                y=rs_df[lai_column + " Unsmoothed"],
                mode="lines",
                name=f"RS {lai_agg_type_name} LAI Unsmoothed",
                line=dict(
                    color="blue",
                    width=3,
                ),
    ...
    fig.add_trace(
        go.Scatter(
            x=cloud_data.index,
            y=cloud_data["Cloud or Snow Percentage"],
            mode="lines",
            name="RS Cloud Coverage %",
    ...
    fig.add_trace(
        go.Scatter(
            x=mean_data.index,
            y=mean_data[f"{crop_name}.Leaf.LAI"],
            mode="lines",
            name="Mean Matched LAI",
    ...
    cloud_snow_percentage = rs_df[(rs_df["interpolated"] == 0) & (rs_df["Cloud or Snow Percentage"] < 100)][
        "Cloud or Snow Percentage"
    ].mean()

    title_text = (
        f"<b>Sim/Real (APSIM/S2-LAI) Matching</b><br>"
...
@click.option(
    ...
    help="Filepath to the filtered CSV.",
)
@click.option(
    "--rs_lai_csv_fpath",
    required=True,
    type=click.Path(exists=True),
    help="Path to remotely sensed LAI CSV file",
)
@click.option(
    "--apsim_db_fpath",
    required=True,
    type=click.Path(exists=True),
    help="Filepath to the APSIM SQLite database.",
)
@click.option(
    "--crop_name",
    required=True,
    type=click.Choice(["wheat", "maize"]),
    help="Crop name to use for LAI lookup in APSIM",
)
...
@click.option(
    ...
    required=False,
    type=click.Path(),
    help="Optional filepath to save the HTML report.",
    default=None,
)
@click.option(
    "--png_fpath",
    required=False,
    type=click.Path(),
    help="Optional filepath to save the PNG report.",
    default=None,
)
