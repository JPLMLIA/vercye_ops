...
@click.option(
    ...
    required=True,
    type=click.Path(),
    help="Filepath where the resulting metrics should be saved. Must be a .csv file.",
)
def cli(estimation_fpath, aggregation_col, referencedata_fpath, out_fpath):
    ...
    if aggregation_col not in estimation_data.columns:
        raise ValueError(
            f"Column '{aggregation_col}' not found in the input estimation csv. Please ensure that the columns are named correctly."
        )
    ...
    if referencedata_fpath:
        ...
        aggregated_data["region"] = aggregated_data["region"].astype(str)
        aggregated_data = aggregated_data.merge(refdata, on="region")
