...
@click.option(
    ...
    help="Path to the input .met file.",
)
@click.option(
    "--output_fpath",
    required=True,
...
@click.option(
    ...
    required=True,
    type=click.Choice(["NASA_POWER", "CHIRPS", "ERA5"]),
    help='Source of precipitation data. "NASA_Power" or "CHIRPS".',
)
@click.option(
    "--precipitation_agg",
    required=True,
    type=click.Choice(["centroid", "mean"]),
    help='Aggregation method for precipitation data. "centroid" or "mean".',
)
@click.option(
    "--fallback_precipitation",
    default=False,
    help="Boolean specifying whether to fallback on nasapower/era5 precipitation data if chirps data is not available.",
)
@click.option(
    "--chirps_file",
    required=False,
    type=click.Path(exists=True),
    default=None,
    help="Path to the chirps parquet file if using chirps.",
)
...
@click.option(
    ...
    default=6,
    show_default=True,
    help="Line number of the column headers (1-based index).",
)
def cli(
    input_fpath,
    output_fpath,
    ...
    """
    data, metadata = plot_weather_data(
        input_fpath,
        precipitation_source,
        precipitation_agg,
        fallback_precipitation,
    ...
    metadata.update({"input_fpath": input_fpath, "output_fpath": output_fpath})
    ...
    if chirps_fpath is not None:
        ...
        if region_name in regions_using_chirps:
            ...
            if not fallback_precipitation:
                raise ValueError(
                    f"No CHIRPS data found for region {region_name}. Please provide a valid region or set fallback_precipitation to True."
                )
...
def create_plots(df, metadata):
    ...
    def add_traces(entire_df, data_type, fig):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df["maxt"],
                mode="lines",
                name=f"Max Temp ({data_type})",
                line=dict(color="red", dash=line_dash, width=0.75),
            ),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(
                ...
                y=df["mint"],
                mode="lines",
                name=f"Min Temp ({data_type})",
                line=dict(color="blue", dash=line_dash, width=0.75),
            ),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Scatter(
                ...
                y=df["meant"],
                mode="lines",
                name=f"Mean Temp ({data_type})",
                line=dict(color="black", dash=line_dash, width=0.75),
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df["radn"],
                mode="lines",
                name=f"Radiance ({data_type})",
                line=dict(color="goldenrod", dash=line_dash, width=1.5),
            ),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df["rain"],
                mode="lines",
                name=f"Precipitation (Corrected) ({data_type})",
                line=dict(color="navy", dash=line_dash, width=1.5),
            ),
            row=3,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df["wind"],
                mode="lines",
                name=f"Surface Wind ({data_type})",
                line=dict(color="purple", dash=line_dash, width=1.5),
            ),
            row=4,
            col=1,
        )
    ...
    fig.add_vline(
        x=metadata["met_meas_end_date"].timestamp()
        * 1000,  # This is a bug workaround: https://github.com/plotly/plotly.py/issues/3065
        row="all",
    ...
    met_header_information = (
        f"Lat {metadata['latitude']}, Lon {metadata['longitude']}, Tav {metadata['tav']}°C, Amp {metadata['amp']}°C"
    )
