...
def compute_global_summary(regions_summary):
    ...
    if "reported_mean_yield_kg_ha" in regions_summary.columns:
        reported_regions_data = regions_summary[
            ~regions_summary["reported_mean_yield_kg_ha"].isna()
        ]

        if regions_summary["reported_mean_yield_kg_ha"].isna().any():
            logger.warning(
                "Some regions have NaN reported yield. Not reporting global aggregated reference."
            )
            logger.warning(
...
def create_map(regions_summary, combined_geojson):
    ...
    norm = mcolors.Normalize(
        vmin=merged["mean_yield_kg_ha"].min(), vmax=merged["mean_yield_kg_ha"].max()
    )
    ...
    ax.set_title(
        "Crop Productivity Overview - Estimated Mean Yield (kg/ha) per Region",
        fontsize=16,
    )
    ax.axis("off")
    return ax
...
def build_section_params(
    ...
    logger.info(
        f"Combined geojson shape: {combined_geojson.shape}. Num regions: {len(regions_geometry_paths)}"
    )
...
def generate_final_report(
    sections, global_summary, metadata, met_config, aggregated_yield_map_preview_path
):
...
def create_final_report(input, output, params, log, wildcards):
    ...
    aggregationsuffix_admincol = params[
        "suffix_admincols"
    ]  # dict of aggregation level suffixes and admin column names
    primary_suffix = params[
        "primary_suffix"
    ]  # should be just primary - is the simulation level suffix
    year = wildcards["year"]
    ...
    for suffix, admin_column_name in aggregationsuffix_admincol.items():
        ...
        aggregated_yield_estimates_patttern = os.path.join(
            regions_dir, f"agg_yield_estimates_{suffix}_*.csv"
        )
        ...
        if not os.path.exists(evaluation_results_path):
            logger.warning(
                f"Evaluation results file not found: {evaluation_results_path}. Skipping."
            )
