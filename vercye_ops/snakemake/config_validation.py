...
def _validate_sim_study_head_dir(config):
    ...
    if not os.listdir(head_dir):
        raise FileNotFoundError(
            f"Simulation head dir {head_dir} is empty, but should contain entries for each year/timepoint"
        )
...
def _validate_lai_time_bounds(config):
    ...
    for year in years:
        ...
        for timepoint in timepoints:
            ...
            if len(bounds) != 2:
                raise ValueError(
                    f"LAI time bounds must have exactly 2 dates for year {year}, timepoint {timepoint}. First one is the start date, the second one the end date."
                )
...
def _validate_lai_files(config):
    ...
    if len(lai_files) == 0:
        raise FileNotFoundError(
            f"No LAI files found matching pattern {pattern}."
    ...
    if len(lai_files) < 5:
        print(
            f"⚠️  Only {len(lai_files)} LAI files found matching pattern {pattern}. "
