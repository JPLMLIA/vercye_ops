...
def convert_shapefile_to_geojson(shp_fpath, projection_crs, admin_name_col, output_head_dir):
    ...
    logger.warning(
        "Ensure all geometries are at the same administrative level! Use the remove_mixed_admin_levels.py script to standardize the shapefile if this is not the case."
    )
